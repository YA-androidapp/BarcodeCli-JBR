var javascriptBarcodeReader = (function (jimp) {
	...
	function createImageData(image) {
		...
		var width = Math.max(image.width || image.naturalWidth, 1);
		var height = Math.max(image.height || image.naturalHeight, 1);
	...
	var code128 = function (lines) {
		...
		if (sumOP % 103 !== WIDTH_TBL.indexOf(letterKey)) {
			return null
		}
	...
	var _2of5 = function (lines, type) {
		...
		if (type === 'interleaved') {
			...
			if (startChar$1 !== 'wwn' || endChar$1 !== 'wnw') {
				return null
			}
